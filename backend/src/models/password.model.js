...
export const createPassword = async (data) => {
  try {
    await db.query(
      ...
      [data.user_id, data.service_name, data.login, data.password_hash, data.iv]
    );
  } catch (error) {
...
export const getAllPasswords = async (user_id) => {
  ...
  const [rows] = await db.query(
    "SELECT * FROM entrees WHERE user_id = ?",
    [user_id]
  );
  return rows;
  } catch (error) {
...
export const updatePasswordById = async (id, data) => {
    try {
        await db.query(
            'UPDATE entrees SET user_id = ?, service_name = ?, login = ?, password_hash = ? WHERE id = ?',
            [
           data.user_id, data.service_name, data.login, data.password_hash, id
            ]
        )
